...
% Example: [Sub]=imsub_fft('ImBS.fits','ImGS.fits');
% Reliable: 2
%--------------------------------------------------------------------------
        ...
        switch lower(InPar.BetaMethod)
            case {'robustfit','ransac'}
                ...
                while (~BetaGammaConverg && IterBeta<=InPar.MaxIterBeta),
                    ...
                    Il0 = Dn(:)>Std_Dn.*3 & Dr(:)>Std_Dr.*3 & InPar.SatPixR(:)==0 & InPar.SatPixN(:)==0;
                    ...
                    switch lower(InPar.BetaMethod)
                        ...
                        case 'ransac'
                            Par = fitlin_ransac(Dr(Il0),Dn(Il0),Dn(Il0),1,100);
                        otherwise
...
%                      input('next')
    ...
    if (InPar.NormS),
         [Back,Std] = mode_fit(Sub(Iim).S);
         Sub(Iim).S = (Sub(Iim).S - Back)./Std;
    end
    if (InPar.NormScorr),
         [Back,Std] = mode_fit(Sub(Iim).Scorr);
         Sub(Iim).Scorr = (Sub(Iim).Scorr - Back)./Std;
    end
    if (InPar.NormD),
         [Back,Std] = mode_fit(Sub(Iim).D);
